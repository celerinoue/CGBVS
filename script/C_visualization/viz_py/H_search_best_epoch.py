...
def main():
    ...
    # load diff_means_ACC data
    data_dir_diff_means = f'viz/{dataset}/{model}/F_corr_acc_important_residues/{seq_name}/acc_dfmeans_by_epoch_{seq_name}_per{threshold}.csv'
    data_diff_means = data_load_diff_means(data_dir_diff_means)

    best_epoch = cal_best_epoch(data_diff_means)



    # set path
    data_dir = f'viz/{dataset}/{model}/A_process/{seq_name}'
    save_dir = f'viz/{dataset}/{model}/H_search_best_epoch/{seq_name}'
...
def data_load_diff_means(data_dir_diff_means):
    data_diff_means = pd.read_csv(data_dir_diff_means, index_col=0)
    return data_diff_means


def cal_best_epoch(df):
    # 50epoch以降を抽出
    ...
    for e in match_epoch_idx:
        ...
        df_ar_epoch_mdiff = df.query(f"epoch <= {int(e)+6} and epoch >= {int(e)-5}")
        ar_epoch_mdiff_mean = np.array(df_ar_epoch_mdiff['diff_means']).mean()
